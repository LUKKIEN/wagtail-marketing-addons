...
find ../src/wagtail_marketing -iname *.po ! -iwholename */en/* -exec msgattrib --translated --no-fuzzy --no-obsolete --no-location -o {} {} \;

# Run compilemessages on each app
for d in $(find ../src/wagtail_marketing -iname *.po | sed 's|\(.*\)/locale.*|\1|' | sort -u);
do
    pushd $d
